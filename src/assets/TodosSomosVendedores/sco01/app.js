(function() {
    ...
    plantillaApp.service('PagesService', function ($http, $rootScope, scormService, $window, textosService) {
        ...
        function getTitlePageStatus(tit){ 
            var resp=false;   
            for (var j = 0; j < _pagesArray.length; j++) {
                if(_pagesArray[j].title == String(tit)){
                    resp = _pagesArray[j].done;
                    break;
                }
        ...
        function getCurrentPageTittle () {
            return _pagesArray[_currentPage - 1].title;
        }


        function getPageStatusTitle(tit){ 
            var status__ = false;
			//console.log("getPageStatusTitle:::: " + _pagesArray.length);
            for (var j = _pagesArray.length-1; j >= 0; j--) {
				//console.log("BUSCAR:::: " + _pagesArray[j].title + " == " + String(tit));
                if(_pagesArray[j].title == String(tit)){
                    status__ = _pagesArray[j].done;
					break;
                }
       ...
       function nextPage() {
        ...
        return {
            ...
            getTitlePageStatus: getTitlePageStatus,
            getPageStatusTitle: getPageStatusTitle,
            getCurrentPageTittle: getCurrentPageTittle,
            setCalif: setCalif,
            getCalif: getCalif,
            setTiempo: setTiempo,
            getTiempo: getTiempo,
       ...
       return {
           ...
           link: function (scope, element, attribute, rootScope) {
               ...
               function onResize(){
                   ...
                   /* VIDEO GULAR */
                    $window.video_player.style.width = 470*sRatio+'px';
                    $window.video_container.style.width = 470*sRatio+'px';

                    $window.video_player.style.height = 264*sRatio+'px';
                    $window.video_container.style.height = 264*sRatio+'px';
				
			//$window.video_player.style.marginLeft = 614*sRatio+'px';
            //$window.video_container.style.marginLeft = 584*sRatio+'px';

                    $window.video_player.style.top = 199*sRatio+'px';
                    //$window.video_container.style.top = 150*sRatio+'px';
                   
                    var centro = parseInt(canvas.style.width) - parseInt($window.video_player.style.width);                   
		    $window.video_player.style.left = parseInt(canvas.style.marginLeft) + (475*sRatio) + "px";				   
          //$window.video_player.style.left = parseInt(canvas.style.marginLeft) + (414*sRatio) + "px";
