(function() {
    ...
    plantillaApp.controller('plantillaAppController', function($scope, $window, PagesService, $rootScope, snapRemote, scormService, gradeFactory, GradeService, ModalsService, $sce, videoService, $timeout, textosService){
        ...
        $scope.videos = [
            {
                sources: [
                    {src: $sce.trustAsResourceUrl("videos/BBVA_DataContent_web.mp4"), type: "video/mp4"}
                ]
            },
            {
                sources: [
                    {src: $sce.trustAsResourceUrl("videos/BBVA_DataQuality_web.mp4"), type: "video/mp4"}
                ]
            },
            {
                sources: [
                    {src: $sce.trustAsResourceUrl("videos/BBVA_DataLineage_web.mp4"), type: "video/mp4"}
                ]
            },
            {
                sources: [
                    {src: $sce.trustAsResourceUrl("videos/BBVA_DataAvailability_web.mp4"), type: "video/mp4"}
                ]
            }
        ];
    ...
    plantillaApp.service('PagesService', function ($http, $rootScope, scormService, $window, textosService) {
        ...
        function init() {
            $http.get('contenido/temario.json').success(function(data) {
                ...
                console.log("CADENA C1: " + get_cadena);
                if(get_cadena){
       ...
       function nextPage() {
        ...
        function setCurrentPage(num){
           // console.log("actualiza currentPage DESDE CURSO 1---- "  + num);
            if($window.parent){
